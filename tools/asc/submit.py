...
• 수정 모드에도 서식 툴바 — 이미 쓴 메모에 체크리스트·제목·인용을 나중에 넣을 수 있습니다
...
def cmd_submit():
    ...
    for extra in existing[1:]:  # 항목 없는 빈 제출 건은 취소도 안 됨(무해) — 최선 노력만
        try: api("PATCH", f"/v1/reviewSubmissions/{extra['id']}", {"data": {"type": "reviewSubmissions", "id": extra["id"], "attributes": {"canceled": True}}})
