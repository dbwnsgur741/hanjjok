...
| 항목 | 값 |
|---|---|
| 앱 이름 (30자) | **한쪽: 초성으로 찾는 메모장** (App Store Connect 확정) — "한쪽" 단독은 iOS 기존 앱(HWP 뷰어)이 선점. Mac 표시명은 그대로 "한쪽" |
...
`ㅅㅇㄷ`만 쳐도 찾아지는 메모장. 메뉴바에서 Option+Space 한 번이면 화면 한쪽에 종이 한 장이 나옵니다. 계정도, 서버도, 동기화도 없이 전부 내 Mac 안에만.
...
## 스크린샷 (Mac: 2880×1800, 4장 — `docs/screenshots/`)

| 순서 | 파일 | 메시지 |
|---|---|---|
| 1 | `01-timeline-light.png` | 초성으로 찾는 맥 메모장 — 타임라인·폴더 칩·체크리스트·태그 |
| 2 | `02-search-light.png` | ㅅㅇㄷ만 쳐도 찾아집니다 — 초성 검색 하이라이트 |
| 3 | `03-timeline-dark.png` | 라이트는 한지, 다크는 먹 |
| 4 | `04-search-dark.png` | 계정도 서버도 없습니다 |

재생성: `tools/screenshots/` 참조. 격리 번들 ID(`kr.hurdlers.HanjjokShots`)로 Debug 빌드 →
`HANJJOK_SNAPSHOT_DIR` 환경변수로 실행 → `seed.py`로 데모 데이터 → `snap.req`로 패널 렌더 → `compose.swift`로 합성.
화면 기록 권한 없이 동작한다(앱이 자기 뷰를 직접 렌더).
